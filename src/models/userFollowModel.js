...
const getFollowersByUserId = async (user_id, res) => {
    try {
        const [rows] = await promisePool.query('SELECT user.name FROM user, user_follow WHERE followed_user_id = ? AND user.id = user_follow.user_id', [user_id]);
        return rows;
    } catch (e) {
        console.error('error', e.message);
        res.status(500).send(e.message);
...
const getFollowedUserByUserId = async (user_id, res) => {
    try {
